...
- **Models**:
  - VoxCPM2 (already pip-installed at `D:\VoxCPM\.venv`) — TTS. Smoke-tested green: 28 s cold, 7.5 s warm, 11.5 GB peak VRAM, releases on `/unload`.
  - Z-Image-Turbo (8-step distilled, 6B, ~12 GB VRAM) at `D:\Z-Image` — image gen. **Real perf on RTX 5070 (sm_120) Windows: ~38 min/image at 1920×1088 BF16** because flash-attn has no Windows wheel for cu128 and Z-Image falls back to native SDPA which lacks fast kernels for sm_120. Mitigation: default local mode to **1024×1024** (~3.5× less compute → ~12 min/image — still slow but workable for lifecycle tests). xformers does NOT help — Z-Image's `AttentionBackend` enum has no xformers backend registered.
  - LTX-2 22B DistilledPipeline + FP8 cast + sequential CPU offload at `D:\LTX-2` — image-to-video. Requires Gemma 3 12B text encoder (gated repo on HuggingFace — needs `HF_TOKEN` with approved license; verified by reading `D:\LTX-2\packages\ltx-trainer\AGENTS.md` — no alternate non-gated encoder exists).
- **Hardware roadmap**:
  - **Today**: RTX 5070 12 GB. VoxCPM2 fits comfortably (~4 GB). Z-Image-Turbo fits but uses ~12 GB (tight). LTX-2 22B requires `enable_sequential_cpu_offload()` (model > VRAM by ~30 GB).
  - **Monday upgrade**: RTX 5080 16 GB. **Same Blackwell sm_120 architecture** — flash-attn / native-SDPA limitations persist (perf issue does NOT auto-fix). Extra 4 GB lets Z-Image run more comfortably (no risk of OOM at full BF16) and reduces LTX-2's CPU-offload aggression. **LTX-2 22B BF16 (~43 GB) still won't fit** — offload required either way.
...
- **Final render**: KENBURNS GPU + RunPod CPU chunk-renderer disabled; local CPU Ken Burns + sequential chunks. Encoder swaps `libx264` → `h264_nvenc` using **the existing `ffmpeg-static@5.3.0` already in `render-api/node_modules/`** — verified to ship with `h264_nvenc`, `hevc_nvenc`, `av1_nvenc` encoders on Windows. **No `gyan.dev` install or `FFMPEG_PATH` env override needed.** Smoke/embers overlays already committed at `D:\historygen\public\overlays\embers.mp4` + `smoke_gray.mp4` (Vercel serves them at `/overlays/...` for production); copied to `local-assets/fx/` for local mode. **Note**: existing `render-video.ts:61-62` hardcoded `historygenai.netlify.app/overlays/...` URLs return 404 today, so production renders silently skip overlays (warn + continue path) — the local-mode change effectively *fixes* this for local dev. Remote-mode overlay URL fix is out-of-scope for this plan.
...
| `render-api/src/lib/runtime-config.ts` | 1–57 | env loader uses plain `process.env`, no validation | Add `LOCAL_INFERENCE`, `LOCAL_VOXCPM2_URL`, `LOCAL_ZIMAGE_URL`, `LOCAL_LTX2_URL`, `LOCAL_ASSETS_DIR`, `LOCAL_ASSETS_BASE_URL` with safe-default fallbacks. Match existing pattern (no Zod). **`FFMPEG_PATH` removed** — `ffmpeg-static@5.3.0` already ships with NVENC encoders on Windows |
