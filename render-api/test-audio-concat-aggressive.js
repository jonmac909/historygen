/**
 * Test with AGGRESSIVE boundary mismatches (like real TTS might produce)
 * This should TRIGGER the glitch detector
 */

const fs = require('fs');

// Generate WAV with INTENTIONAL large amplitude at boundary
function generateAggressiveTTSWav(durationSeconds, endAmplitude, sampleRate = 24000) {
  ...
  // Generate speech-like audio that ENDS at specified amplitude
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    let value = Math.floor(f1 + f2 + f3 + noise);

    // Force last few samples to target amplitude
    if (i >= numSamples - 5) {
      value = endAmplitude;
    }
...
// Run test
console.log('=== Testing AGGRESSIVE Boundary Mismatches ===\n');

console.log('Generating chunks with INTENTIONAL large boundary jumps...');
const chunk1 = generateAggressiveTTSWav(0.3, 8000);   // Ends at +8000
const chunk2 = generateAggressiveTTSWav(0.3, -8000);  // Ends at -8000
const chunk3 = generateAggressiveTTSWav(0.3, 8000);   // Ends at +8000
...
fs.writeFileSync('/tmp/test-aggressive.wav', combined);
console.log('\n💾 Test audio saved to /tmp/test-aggressive.wav');
console.log('   Listen with: afplay /tmp/test-aggressive.wav');
